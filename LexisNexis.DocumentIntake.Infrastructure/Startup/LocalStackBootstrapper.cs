...
public static class LocalStackBootstrapper
{
    public static async Task EnsureRunningAsync(CancellationToken ct = default)
    {
        ...
        Log.Information("Starting LocalStack via docker compose...");
        ...
        try
        {
            ...
            {
                Log.Warning("docker compose exited with code {Code}: {Error}", process.ExitCode, stderrTask.Result);
                return;
            }
        ...
        {
            Log.Warning(ex, "Could not auto-start LocalStack — start it manually with: docker compose up localstack -d");
        }
    }
    ...
    private static async Task WaitForHealthyAsync(CancellationToken ct)
    {
        ...
        for (var attempt = 1; attempt <= 30; attempt++)
        {
            try
            {
                ...
                {
                    Log.Information("LocalStack is ready.");
                    return;
                }
            }
            catch
        ...
        Log.Warning("LocalStack did not become healthy within 30s.");
    }
